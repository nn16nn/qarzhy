...
var DB = { tx:[], goals:[], accounts:[], btx:[], debts:[], recur:[], budgets:{}, lastBackup:null, lang:'kk', theme:'auto', rate:null, home:null };
...
function load(){
  try{
    ...
    if(raw){
      ...
      DB.lastBackup = d.lastBackup||null; DB.rate = d.rate||null; DB.home = d.home||null;
...
function applyDB(d){
  ...
  DB.theme = d.theme || 'auto'; DB.rate = d.rate || null; DB.home = d.home || null;
...
function render(){
  ...
  /* --- басты беттің карталары мен инвестиция --- */
  applyHome();
  renderHomeWidgets();
  renderInvest();
  drawHomeSet();
...
var HOME_ON_DEF = ['finance', 'quick', 'invest', 'import', 'recent'];
...
function homeCfg(){
  ...
  WIDGETS.forEach(function(w){
    if(h.order.indexOf(w[0]) === -1){ h.order.push(w[0]); h.off[w[0]] = 1; }
  });
